<template>
  <div class="page-shell console-shell">
    <aside class="console-side">
      ...
      <el-button class="side-link logout-btn" @click="logout"
        >退出登录</el-button
      >
      <RouterLink class="side-link" to="/interview/hr/processes"
        >线上面试</RouterLink
      >
    </aside>
    ...
    <main class="console-main">
      ...
      <section v-if="activeTab === 'audit'" class="page-card">
        ...
        <el-form :model="auditFilter" label-position="top" class="filter-grid">
          <el-form-item label="日志分类"
            ><el-select v-model="auditFilter.moduleCode" clearable
              ><el-option label="后台管理日志" value="ADMIN" /><el-option
                label="面试流程日志"
                value="INTERVIEW" /><el-option
                label="报名者投递日志"
                value="RECRUITMENT" /></el-select
          ></el-form-item>
          <el-form-item label="动作"
            ><el-input
              v-model="auditFilter.actionCode"
              placeholder="如 审批AI / 投递报名 / 重置密码"
          /></el-form-item>
          <el-form-item label="关键词"
            ><el-input
              v-model="auditFilter.keyword"
              placeholder="操作人 / 对象ID / 详情"
          /></el-form-item>
        ...
        <el-form :model="userForm" label-position="top" class="form-grid">
          ...
          <el-form-item label="角色"
            ><el-select v-model="userForm.roleCode"
              ><el-option
                v-if="isItAdmin"
                label="IT管理员"
                value="IT_ADMIN" /><el-option
                v-if="isItAdmin"
                label="HR管理员"
                value="HR_ADMIN" /><el-option
                label="HR用户"
                value="HR_USER" /><el-option
                label="面试者"
                value="INTERVIEWEE" /></el-select
          ></el-form-item>
          ...
          <el-form-item label="新密码"
            ><el-input
              ...
              type="password"
              show-password
              placeholder="至少6位，仅重置时填写"
          /></el-form-item>
        </el-form>
        <div class="action-row">
          <el-button type="primary" @click="saveUser">保存用户</el-button
          ><el-button type="warning" @click="resetUserPassword"
            >重置密码</el-button
          >
        </div>
      </section>
      ...
      <section v-if="activeTab === 'departments'" class="page-card">
        ...
        <div class="sub-tabs">
          <button
            :class="{ active: departmentMode === 'create' }"
            @click="showCreateDepartment"
          >
            新增部门
          </button>
          <button
            :class="{ active: departmentMode === 'query' }"
            @click="departmentMode = 'query'"
          >
            查询部门
          </button>
          <button
            :class="{ active: departmentMode === 'edit' }"
            @click="departmentMode = 'edit'"
          >
            修改部门
          </button>
        </div>
        <template
          v-if="departmentMode === 'create' || departmentMode === 'edit'"
        >
          <el-form
          ...
          >
            ...
            <el-form-item label="部门负责人"
              ><el-select
                v-model="departmentForm.managerEmployeeId"
                clearable
                filterable
                placeholder="从员工列表选择负责人"
                ><el-option
            ...
            /></el-form-item>
            <el-form-item label="部门职能简介" class="wide"
              ><el-input
          ...
          <div class="action-row">
            <el-button type="primary" @click="saveDepartment">{{
              departmentMode === "create" ? "新增部门" : "保存修改"
            }}</el-button
        ...
        <template v-if="departmentMode === 'query'">
          ...
          >
            <el-form-item label="上级部门"
              ><el-select
                v-model="departmentFilter.parentDepartmentId"
                clearable
                ><el-option
            ...
            ></el-form-item>
            <el-form-item label="部门状态"
              ><el-select v-model="departmentFilter.status" clearable
                ><el-option label="启用" :value="1" /><el-option
                  label="停用"
                  :value="0" /></el-select
            ></el-form-item>
            <el-form-item label="关键词"
              ><el-input
                v-model="departmentFilter.keyword"
                placeholder="部门名称 / 编码 / 简介"
            /></el-form-item>
            <el-form-item label="操作"
              ><div class="filter-actions">
                <el-button type="primary" @click="loadDepartments"
                  >查询</el-button
                ><el-button @click="resetDepartmentFilter">重置</el-button>
              </div></el-form-item
            >
          </el-form>
          <el-table
            ...
            <el-table-column label="操作" width="100"
              ><template #default="scope"
                ><el-button
                  text
                  type="danger"
                  @click.stop="deleteDepartment(scope.row.id)"
                  >删除</el-button
                ></template
              ></el-table-column
            >
      ...
      <section v-if="activeTab === 'employees'" class="page-card">
        ...
        <div class="sub-tabs">
          <button
            :class="{ active: employeeMode === 'create' }"
            @click="showCreateEmployee"
          >
            新增员工</button
          ><button
            :class="{ active: employeeMode === 'query' }"
            @click="employeeMode = 'query'"
          >
            查询员工</button
          ><button
            :class="{ active: employeeMode === 'edit' }"
            @click="employeeMode = 'edit'"
          >
            修改员工
          </button>
        </div>
          ...
          <el-form :model="employeeForm" label-position="top" class="form-grid">
            <el-form-item label="工号"
              ><el-input
                ...
                placeholder="查询和修改时作为业务主键"
            /></el-form-item>
            <el-form-item label="姓名"
              ><el-input v-model="employeeForm.fullName"
            /></el-form-item>
            <el-form-item label="身份证号"
              ><el-input v-model="employeeForm.idCardNo"
            /></el-form-item>
            <el-form-item label="手机号"
              ><el-input v-model="employeeForm.mobilePhone"
            /></el-form-item>
            <el-form-item label="招聘专业"
              ><el-input v-model="employeeForm.recruitmentMajor"
            /></el-form-item>
            <el-form-item label="岗位"
              ><el-input v-model="employeeForm.positionName"
            /></el-form-item>
            ...
            <el-form-item label="员工状态"
              ><el-select v-model="employeeForm.employmentStatus"
                ><el-option label="待入职" :value="0" /><el-option
                  label="已入职"
                  :value="1" /><el-option label="停用" :value="2" /><el-option
                  label="已离职"
                  :value="3" /></el-select
            ></el-form-item>
            <el-form-item label="银行卡号"
              ><el-input v-model="employeeForm.bankAccountNo"
            /></el-form-item>
            <el-form-item label="开户银行"
              ><el-input v-model="employeeForm.bankName"
            /></el-form-item>
          </el-form>
          <div class="action-row">
            <el-button type="primary" @click="saveEmployee">{{
              employeeMode === "create" ? "新增员工" : "保存修改"
            }}</el-button
        ...
        <template v-if="employeeMode === 'query'">
          ...
          >
            <el-form-item label="直属部门"
              ><el-select v-model="employeeFilter.departmentId" clearable
                ><el-option
                  v-for="item in departments"
                  :key="item.id"
                  :label="item.departmentName"
            ...
            ></el-form-item>
            <el-form-item label="员工状态"
              ><el-select v-model="employeeFilter.employmentStatus" clearable
                ><el-option label="待入职" :value="0" /><el-option
                  label="已入职"
                  :value="1" /><el-option label="停用" :value="2" /><el-option
                  label="已离职"
            ...
            /><el-table-column label="操作" width="100"
              ><template #default="scope"
                ><el-button
                  text
                  type="danger"
                  @click.stop="deleteEmployee(scope.row.id)"
                  >删除</el-button
                ></template
              ></el-table-column
            ></el-table
      ...
      <section v-if="activeTab === 'bindings'" class="page-card">
        ...
        <el-form :model="bindingForm" label-position="top" class="form-grid"
          ><el-form-item label="模块"
            ><el-select v-model="bindingForm.moduleCode"
              ><el-option label="招聘" value="RECRUITMENT" /><el-option
                label="绩效"
                value="PERFORMANCE" /><el-option
                label="面试"
                value="INTERVIEW" /></el-select></el-form-item
          ><el-form-item label="业务类型"
            ><el-input v-model="bindingForm.businessType" /></el-form-item
          ><el-form-item label="员工"
            ><el-select v-model="bindingForm.employeeId" clearable
              ><el-option
                v-for="item in employees"
                :key="item.id"
                :label="item.fullName"
          ...
          ><el-form-item label="部门"
            ><el-select v-model="bindingForm.departmentId" clearable
              ><el-option
                v-for="item in departments"
                :key="item.id"
                :label="item.departmentName"
          ...
          ><el-form-item label="外部引用"
            ><el-input v-model="bindingForm.externalRef" /></el-form-item
          ><el-form-item label="状态"
            ><el-input v-model="bindingForm.bindingStatus" /></el-form-item
        ></el-form>
        ...
        <el-table
          ...
          stripe
          class="data-table"
          @row-click="openBinding"
          ><el-table-column prop="moduleCode" label="模块" /><el-table-column
            prop="businessType"
            label="业务类型" /><el-table-column
            prop="employeeName"
            label="员工" /><el-table-column prop="departmentName" label="部门"
        /></el-table>
      </section>
      ...
      <section v-if="activeTab === 'recruitment'" class="page-card">
        ...
        <div class="sub-tabs">
          <button
            :class="{ active: recruitmentMode === 'jobCreate' }"
            @click="showCreateJob"
          >
            新增岗位</button
          ><button
            :class="{ active: recruitmentMode === 'jobQuery' }"
            @click="recruitmentMode = 'jobQuery'"
          >
            查询岗位</button
          ><button
            :class="{ active: recruitmentMode === 'jobEdit' }"
            @click="recruitmentMode = 'jobEdit'"
          >
            修改岗位</button
          ><button
            :class="{ active: recruitmentMode === 'candidates' }"
            @click="recruitmentMode = 'candidates'"
          >
            候选人信息
          </button>
        </div>
        <template
          v-if="
            recruitmentMode === 'jobCreate' || recruitmentMode === 'jobEdit'
          "
        >
          <el-form :model="jobForm" label-position="top" class="form-grid"
            ><el-form-item label="岗位名称"
              ><el-input v-model="jobForm.jobTitle" /></el-form-item
            ><el-form-item label="岗位编码"
              ><el-input v-model="jobForm.jobCode" /></el-form-item
            ><el-form-item label="招聘部门"
              ><el-select
                v-model="jobForm.departmentName"
                filterable
                placeholder="从数据库部门中选择"
                ><el-option
                  v-for="item in departments"
                  :key="item.id"
                  :label="item.departmentName"
                  :value="item.departmentName" /></el-select></el-form-item
            ><el-form-item label="工作地点"
              ><el-input v-model="jobForm.workLocation" /></el-form-item
            ><el-form-item label="岗位类型"
              ><el-input v-model="jobForm.jobType" /></el-form-item
            ><el-form-item label="招聘人数"
              ><el-input-number
                v-model="jobForm.headcount"
                :min="1" /></el-form-item
            ><el-form-item label="薪资范围"
              ><el-input v-model="jobForm.salaryRange" /></el-form-item
            ><el-form-item label="状态"
              ><el-select v-model="jobForm.status"
                ><el-option label="开放" :value="1" /><el-option
                  label="关闭"
                  :value="0" /></el-select></el-form-item
            ><el-form-item label="岗位职责" class="wide"
              ><el-input
                v-model="jobForm.responsibilities"
                type="textarea"
                :rows="3" /></el-form-item
            ><el-form-item label="任职要求" class="wide"
              ><el-input
          ...
          <div class="action-row">
            <el-button type="primary" @click="saveJob">{{
              recruitmentMode === "jobCreate" ? "新增岗位" : "保存修改"
            }}</el-button
        ...
        <template v-if="recruitmentMode === 'jobQuery'">
          <el-form :model="jobFilter" label-position="top" class="filter-grid">
            <el-form-item label="招聘部门"
              ><el-select v-model="jobFilter.departmentName" clearable
                ><el-option
                  v-for="item in departments"
                  :key="item.id"
                  :label="item.departmentName"
                  :value="item.departmentName" /></el-select
            ></el-form-item>
            <el-form-item label="岗位类型"
              ><el-input
            ...
            <el-form-item label="岗位状态"
              ><el-select v-model="jobFilter.status" clearable
                ><el-option label="开放" :value="1" /><el-option
                  label="关闭"
                  :value="0" /></el-select
            ></el-form-item>
            <el-form-item label="关键词"
              ><el-input
                v-model="jobFilter.keyword"
                placeholder="岗位 / 编码 / 地点 / 薪资"
            /></el-form-item>
            <el-form-item label="操作"
              ><div class="filter-actions">
          ...
          <el-table :data="jobs" stripe class="data-table" @row-click="openJob"
            ><el-table-column
              prop="jobTitle"
              label="岗位"
              min-width="140"
        ...
        <template v-if="recruitmentMode === 'candidates'">
          ...
          >
            <el-form-item label="岗位"
              ><el-select v-model="candidateFilter.jobId" clearable
                ><el-option
                  v-for="job in jobs"
                  :key="job.id"
                  :label="job.jobTitle"
            ...
            <el-form-item label="投递状态"
              ><el-select v-model="candidateFilter.status" clearable
                ><el-option label="已投递" value="SUBMITTED" /><el-option
                  label="面试中"
                  value="INTERVIEWING" /></el-select
            ></el-form-item>
            <el-form-item label="面试阶段"
              ><el-select
                v-model="candidateFilter.interviewStageStatus"
                clearable
                ><el-option label="简历待查" value="简历待查" /><el-option
                  label="AI面"
                  value="AI面" /><el-option
                  label="AI待审批"
                  value="AI待审批" /><el-option
                  label="视频面"
                  value="视频面" /><el-option
                  label="视频待审批"
                  value="视频待审批" /><el-option
                  label="线下面"
                  value="线下面" /><el-option
                  label="已通过"
                  value="已通过" /><el-option
            ...
            <el-form-item label="关键词"
              ><el-input
                v-model="candidateFilter.keyword"
                placeholder="姓名 / 手机 / 专业 / 学校"
            /></el-form-item>
            <el-form-item label="操作"
              ><div class="filter-actions">
                <el-button type="primary" @click="loadCandidates"
                  >查询</el-button
                ><el-button @click="resetCandidateFilter">重置</el-button>
              </div></el-form-item
            >
          </el-form>
          <el-table
            ...
            ><el-table-column
              ...
              ><template #default="scope"
                ><span>{{
                  scope.row.resumeLlmScore ??
                  resumeLlmStatusLabel(scope.row.resumeLlmStatus)
                }}</span></template
              ></el-table-column
            ><el-table-column
              prop="interviewProcessId"
              label="流程流水号"
              min-width="120" /><el-table-column label="简历" min-width="150"
              ><template #default="scope"
                ><a
                  v-if="scope.row.resumeFileId"
                  class="resume-link"
                  :href="resumeUrl(scope.row.resumeFileId)"
                  target="_blank"
                  @click.stop
                  >{{ scope.row.resumeFileName || "查看简历" }}</a
                ><span v-else>未上传</span></template
              ></el-table-column
            ><el-table-column label="操作" width="460"
              ><template #default="scope"
                ><el-button text @click.stop="openCandidate(scope.row)"
                  >查看详情</el-button
                ><el-button
                  text
                  :disabled="!canReevaluateResumeLlm(scope.row)"
                  @click.stop="reevaluateResumeLlm(scope.row.id)"
                  >{{ resumeLlmReevaluateLabel(scope.row) }}</el-button
                ><el-button
                  text
                  @click.stop="startCandidateInterview(scope.row)"
                  >发起面试</el-button
                ><el-button
                  text
                  type="danger"
                  @click.stop="rejectCandidateResume(scope.row.id)"
                  >简历拒绝</el-button
                ><el-button
                  text
                  type="danger"
                  @click.stop="deleteCandidate(scope.row.id)"
                  >删除候选人</el-button
                ></template
              ></el-table-column
...
const dashboard = reactive({
  ...
  recruitmentBindingCount: 0,
  performanceBindingCount: 0,
});
...
const bindingForm = reactive({
  ...
  businessType: "EMPLOYEE_SYNC",
  employeeId: null,
  departmentId: null,
  externalRef: "",
  bindingStatus: "ACTIVE",
  payload: '{"source":"frontend-demo"}',
});
...
const availableParentDepartments = computed(() =>
  departments.value.filter((item) => item.id !== departmentForm.id),
);
  ...
  [
    {
      key: "admin",
      title: "后台管理日志",
      items: auditLogs.value.filter(
        (item) => !["INTERVIEW", "RECRUITMENT"].includes(item.moduleCode),
      ),
    },
    {
      key: "interview",
      title: "面试流程日志",
      items: auditLogs.value.filter((item) => item.moduleCode === "INTERVIEW"),
    },
    {
      key: "recruitment",
      title: "报名者投递日志",
      items: auditLogs.value.filter(
        (item) => item.moduleCode === "RECRUITMENT",
      ),
    },
  ].filter(
    (group) =>
      !auditFilter.moduleCode ||
      group.key === auditFilter.moduleCode.toLowerCase(),
  ),
);
...
function localizeDetail(value) {
  ...
  return Object.entries({
    ...
    ...targetLabels,
    ...detailLabels,
  }).reduce((text, [key, label]) => String(text).replaceAll(key, label), value);
}
...
async function loadDashboard() {
  try {
    Object.assign(dashboard, (await hrApi.getDashboard()).data);
  } catch (error) {
    fail(error);
  }
}
async function loadDepartments() {
  try {
    departments.value = (
      await hrApi.listDepartments(cleanParams(departmentFilter))
    ).data;
  } catch (error) {
    fail(error);
  }
...
async function loadEmployees() {
  try {
    employees.value = (
      await hrApi.listEmployees(cleanParams(employeeFilter))
    ).data;
  } catch (error) {
    fail(error);
  }
...
async function loadBindings() {
  try {
    bindings.value = (await hrApi.listBindings()).data;
  } catch (error) {
    fail(error);
  }
}
async function loadJobs() {
  try {
    jobs.value = (
      await recruitmentApi.listAdminJobs(cleanParams(jobFilter))
    ).data;
  } catch (error) {
    fail(error);
  }
...
async function loadCandidates() {
  try {
    candidates.value = (
      await recruitmentApi.listCandidates(cleanParams(candidateFilter))
    ).data;
  } catch (error) {
    fail(error);
  }
...
async function loadUsers() {
  if (!(isItAdmin.value || isHrAdmin.value)) return;
  try {
...
async function loadAuditLogs() {
  ...
  try {
    const params = cleanParams({
      ...auditFilter,
      actionCode: resolveActionCode(auditFilter.actionCode),
    });
    auditLogs.value = (await authApi.listAuditLogs(params)).data;
  } catch (error) {
    fail(error);
  }
}
...
async function saveDepartment() {
  try {
    await hrApi.saveDepartment({ ...departmentForm });
...
async function saveEmployee() {
  try {
    await hrApi.saveEmployee({ ...employeeForm });
...
async function saveBinding() {
  try {
    await hrApi.saveBinding({ ...bindingForm });
...
function showCreateJob() {
  resetJobForm();
  recruitmentMode.value = "jobCreate";
}
function editJob(row) {
  Object.assign(jobForm, row);
  recruitmentMode.value = "jobEdit";
}
async function saveJob() {
  try {
    await recruitmentApi.saveJob({ ...jobForm });
    ElMessage.success("招聘岗位已保存");
    resetJobForm();
...
function resetAuditFilter() {
  Object.assign(auditFilter, { moduleCode: "", actionCode: "", keyword: "" });
  loadAuditLogs();
}
function resetDepartmentFilter() {
  Object.assign(departmentFilter, {
    parentDepartmentId: null,
    status: null,
    keyword: "",
  });
...
function openBinding(row) {
  ...
  router.push(`/admin/bindings/${row.id}`);
}
function openJob(row) {
  router.push(`/admin/recruitment/jobs/${row.id}`);
}
function openCandidate(row) {
  router.push(`/admin/recruitment/candidates/${row.id}`);
}
function resumeUrl(id) {
  return recruitmentApi.getResumeUrl(id);
}
...
function canReevaluateResumeLlm(candidate) {
  return candidate?.resumeLlmStatus !== "PENDING";
}
function resumeLlmReevaluateLabel(candidate) {
  return canReevaluateResumeLlm(candidate) ? "AI简历重评" : "评分中不可重评";
}
async function startCandidateInterview(candidate) {
  try {
    const userList = (
      await authApi.listUsers({
        roleCode: "INTERVIEWEE",
        keyword: candidate.mobilePhone,
      })
    ).data;
    const interviewee = userList.find(
      (item) => item.mobilePhone === candidate.mobilePhone,
    );
    if (!interviewee) {
      ElMessage.warning("未找到对应面试者账号，请先注册并完善资料");
      return;
    }
    await interviewApi.startProcess({
      recruitmentCandidateId: candidate.id,
      intervieweeUserId: interviewee.id,
...
function editUser(row) {
  Object.assign(userForm, row, { newPassword: "" });
}
...
function resolveActionCode(value) {
  if (!value) return "";
  return (
    Object.entries(actionLabels).find(
      ([key, label]) => key === value || label.includes(value),
    )?.[0] || value
  );
}
...
.console-side button.active {
  background: #f8f5ef;
  border-color: rgba(240, 182, 111, 0.58);
  color: #102532;
}
...
.logout-btn {
  margin-top: auto;
  background: rgba(184, 67, 67, 0.15) !important;
  border-color: rgba(184, 67, 67, 0.3) !important;
  color: #f4efe7 !important;
}

.logout-btn:hover {
  background: rgba(184, 67, 67, 0.25) !important;
  border-color: rgba(184, 67, 67, 0.5) !important;
}
