  ...
  <div class="page-shell">
    <section class="page-card">
      ...
      <section v-if="activeTab === 'system'" class="surface">
        <h3>系统配置</h3>
        <p class="serial-line">配置写入服务器 .env 文件，部分配置需要重启服务生效。敏感字段显示为掩码，保存时留空则不覆盖。</p>
        <div class="llm-config-grid">
          ...
          <div class="surface inner-surface">
            <h3>阿里云语音识别 STT</h3>
            <el-form label-position="top" class="form-grid">
              <el-form-item label="NLS AppKey"><el-input v-model="systemConfig.ALIYUN_STT_APP_KEY" /></el-form-item>
              <el-form-item label="NLS Endpoint"><el-input v-model="systemConfig.ALIYUN_STT_ENDPOINT" placeholder="nls-gateway-cn-shanghai.aliyuncs.com" /></el-form-item>
            </el-form>
          </div>
          <div class="surface inner-surface">
            <h3>阿里云 OSS</h3>
            <el-form label-position="top" class="form-grid">
              <el-form-item label="Bucket名称"><el-input v-model="systemConfig.ALIYUN_OSS_BUCKET_NAME" /></el-form-item>
              <el-form-item label="Endpoint"><el-input v-model="systemConfig.ALIYUN_OSS_ENDPOINT" /></el-form-item>
            </el-form>
          </div>
          <div class="surface inner-surface">
            <h3>视频面试 / WebRTC</h3>
            <el-form label-position="top" class="form-grid">
              <el-form-item label="FFmpeg路径"><el-input v-model="systemConfig.INTERVIEW_VIDEO_FFMPEG_PATH" /></el-form-item>
              <el-form-item label="视频编码"><el-input v-model="systemConfig.INTERVIEW_VIDEO_VIDEO_CODEC" /></el-form-item>
              <el-form-item label="音频编码"><el-input v-model="systemConfig.INTERVIEW_VIDEO_AUDIO_CODEC" /></el-form-item>
              <el-form-item label="STUN URLs"><el-input v-model="systemConfig.INTERVIEW_STUN_URLS" /></el-form-item>
              <el-form-item label="TURN URLs"><el-input v-model="systemConfig.INTERVIEW_TURN_URLS" /></el-form-item>
              <el-form-item label="TURN Username"><el-input v-model="systemConfig.INTERVIEW_TURN_USERNAME" /></el-form-item>
              <el-form-item label="TURN Credential"><el-input v-model="systemConfig.INTERVIEW_TURN_CREDENTIAL" type="password" show-password /></el-form-item>
            </el-form>
          </div>
          <div class="surface inner-surface">
            <h3>数据库</h3>
            <el-form label-position="top" class="form-grid">
              <el-form-item label="数据库类型"><el-input v-model="systemConfig.DB_TYPE" placeholder="sqlite / mysql / pgsql" /></el-form-item>
              <el-form-item label="数据库URL"><el-input v-model="systemConfig.DB_URL" /></el-form-item>
              <el-form-item label="用户名"><el-input v-model="systemConfig.DB_USERNAME" /></el-form-item>
              <el-form-item label="密码"><el-input v-model="systemConfig.DB_PASSWORD" type="password" show-password placeholder="留空则不覆盖" /></el-form-item>
            </el-form>
          </div>
          <div class="surface inner-surface">
            <h3>简历 OCR</h3>
            <el-form label-position="top" class="form-grid">
              <el-form-item label="启用OCR"><el-input v-model="systemConfig.RESUME_OCR_ENABLED" placeholder="true / false" /></el-form-item>
              <el-form-item label="Tesseract路径"><el-input v-model="systemConfig.RESUME_OCR_TESSERACT_PATH" /></el-form-item>
              <el-form-item label="识别语言"><el-input v-model="systemConfig.RESUME_OCR_LANGUAGE" placeholder="chi_sim+eng" /></el-form-item>
              <el-form-item label="DPI"><el-input v-model="systemConfig.RESUME_OCR_DPI" placeholder="200" /></el-form-item>
              <el-form-item label="最大页数"><el-input v-model="systemConfig.RESUME_OCR_MAX_PAGES" placeholder="5" /></el-form-item>
            </el-form>
          </div>
          <div class="surface inner-surface">
            <h3>SMTP 邮件</h3>
            <el-form label-position="top" class="form-grid">
              ...
              <el-form-item label="端口"><el-input v-model="systemConfig.SMTP_PORT" placeholder="465" /></el-form-item>
              ...
              <el-form-item label="发件人地址"><el-input v-model="systemConfig.SMTP_FROM" placeholder="noreply@example.com" /></el-form-item>
              <el-form-item label="SSL"><el-input v-model="systemConfig.SMTP_SSL" placeholder="true / false" /></el-form-item>
            </el-form>
          </div>
        </div>
        ...
        <div class="llm-config-grid">
          ...
          <div class="surface inner-surface">
            ...
            <el-form :model="videoSummaryLlmForm" label-position="top" class="form-grid">
              ...
              <el-form-item label="会议概要提示词" class="wide"><el-input v-model="videoSummaryLlmForm.scoringRulePrompt" type="textarea" :rows="4" placeholder="例如：输出会议概要、关键回答、候选人优势、风险点和后续建议" /></el-form-item>
            </el-form>
            <div class="action-row"><el-button type="primary" @click="saveRoleLlmConfig(videoSummaryLlmForm, 'VIDEO_SUMMARY')">保存视频概要模型</el-button><span class="serial-line">API Key：{{ videoSummaryKeyLabel }}</span></div>
          </div>
        </div>
        ...
        <div v-if="selectedProcess" class="process-workbench">
          ...
          <section class="workbench-panel video-panel">
            ...
            <div class="summary-box">
              ...
              <p v-if="selectedProcess.summaryText"><b>会议概要：</b>{{ selectedProcess.summaryText }}</p>
              <p v-if="selectedProcess.transcriptText"><b>转写文本：</b>{{ selectedProcess.transcriptText }}</p>
              <p v-if="!selectedProcess.summaryText && !selectedProcess.transcriptText">录制拼接完成后自动抽取音频、转写并生成概要。</p>
            </div>
          </section>
          <section class="workbench-panel ai-question-panel">
            <h3>AI问答题号</h3>
            <div v-if="selectedProcess.aiRecordingFileName" class="serial-line">
              <a :href="interviewApi.getAiRecordingUrl(selectedProcess.id)" target="_blank" class="video-link">查看AI面试录像</a>
            </div>
...
async function loadSystemConfig() { try { const res = (await systemApi.getConfig()).data; Object.assign(systemConfig, res) } catch (e) { fail(e) } }
async function saveSystemConfig() {
  savingSystemConfig.value = true
  try {
    const updates = {}
    for (const [k, v] of Object.entries(systemConfig)) { if (v !== '' && v !== '****') updates[k] = v }
    await systemApi.saveConfig(updates)
    await loadSystemConfig()
    ElMessage.success('系统配置已保存')
  } catch (e) { fail(e) } finally { savingSystemConfig.value = false }
}
async function loadAll() {
  try {
    ...
    if (isItAdmin.value) {
      ...
      llmConfigs.value = []
      if (activeTab.value === 'llm' || activeTab.value === 'system') router.replace('/interview/hr/knowledge-bases')
    }
...
function openLlmConfig(row) { editLlmConfig(row); router.push('/interview/hr/system') }
...
function createLlmForm(role) { return { id: null, configName: role === 'SCORER' ? '评分模型' : role === 'RESUME_REVIEW' ? '简历初筛模型' : role === 'VIDEO_SUMMARY' ? '视频概要模型' : '面试官模型', modelRole: role, baseUrl: '', apiKey: '', modelName: '', promptTemplate: '', scoringRulePrompt: '', status: 1 } }
