...
public class PayrollController {
    ...
    @DeleteMapping("/inputs/{kind}/{employeeId}/{salaryMonth}") public ApiResponse<Void> deleteInput(Authentication auth,@PathVariable String kind,@PathVariable Long employeeId,@PathVariable String salaryMonth){SessionUserVO user=user(auth);payrollService.deleteInput(kind,employeeId,salaryMonth);audit(user,kind.toUpperCase()+"_DELETE",employeeId,salaryMonth);return ApiResponse.success("deleted",null);}
    @PostMapping("/{employeeId}/{salaryMonth}/lock") public ApiResponse<Void> lock(Authentication auth,@PathVariable Long employeeId,@PathVariable String salaryMonth){SessionUserVO user=user(auth);payrollService.setLocked(employeeId,salaryMonth,true);audit(user,"PAYROLL_LOCK",employeeId,salaryMonth);return ApiResponse.success("locked",null);}
    ...
    @DeleteMapping("/{employeeId}/{salaryMonth}") public ApiResponse<Void> deletePayroll(Authentication auth,@PathVariable Long employeeId,@PathVariable String salaryMonth){SessionUserVO user=user(auth);payrollService.deletePayroll(employeeId,salaryMonth);audit(user,"PAYROLL_DELETE",employeeId,salaryMonth);return ApiResponse.success("deleted",null);}
