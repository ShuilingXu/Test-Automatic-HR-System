...
public final class PayrollCalculations {
    ...
    public static BigDecimal growth(BigDecimal current, BigDecimal previous, boolean salaryConfirmed) { if(!salaryConfirmed||previous==null||previous.signum()==0)return null;return money(money(current).subtract(money(previous)).multiply(BigDecimal.valueOf(100)).divide(money(previous),2,RoundingMode.HALF_UP)); }
