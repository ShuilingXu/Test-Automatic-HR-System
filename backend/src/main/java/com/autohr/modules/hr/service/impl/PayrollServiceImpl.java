...
public class PayrollServiceImpl implements PayrollService {
    ...
    public List<PayrollVO> listPayroll(String salaryMonth, Long employeeId) {
        ...
        return jdbc.queryForList(sql, args.toArray()).stream().map(this::toPayroll).toList();
    }
    ...
    private PayrollVO calculate(Map<String, Object> employee, YearMonth target) {
        ...
        for (YearMonth month = start; !month.isAfter(target); month = month.plusMonths(1)) {
            ...
            if (month.equals(target)) {
                ...
                vo.setEmployeeId(id); vo.setEmployeeCode(text(employee.get("employee_code"))); vo.setEmployeeName(text(employee.get("full_name"))); vo.setIdCardNo(text(employee.get("id_card_no"))); vo.setSalaryMonth(month.toString());
